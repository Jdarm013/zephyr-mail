...
A self-hosted mail server, designed, built, and run in production on my own domain (`zephyrlink.net`) — real internet email, sent and received, not a localhost demo. Built with Spring Boot, and hardened through a deliberate security self-audit rather than assumed to be safe.
...
- Proof-of-work gated login — the client computes a SHA-256 hash meeting a difficulty prefix (via the Web Crypto API) before a login POST is even accepted, raising the cost of automated credential-stuffing.
...
- Payload sanitization on message bodies to prevent stored XSS, without mangling legitimate `@` / `&` / `<` / `>` characters in addresses or passwords elsewhere in the app.
...
## Built by auditing my own work

Before calling this "done," I went through the app looking for real problems instead of assuming it was secure because I wrote it. That audit found and fixed, among others:
...
- **Testing:** verified so far through manual code review, live log inspection, and running as my actual production mail server, rather than a CI-enforced automated suite — added test coverage is the next major milestone.
- **Storage:** H2 file-based storage, which fits a personal-scale deployment; a move to a networked database would come with multi-tenant support.
- **Availability:** single-instance deployment today, with high-availability failover as a future goal rather than a current guarantee.

## Why I built this

I wanted hands-on experience with a genuinely complex, multi-system problem — SMTP handling, deliverability, and the security surface a login form, a compose box, and an inbound webhook all quietly expose — and to prove to myself I could design and reason through something like that carefully, with the goal of not failing, even on a first attempt at this class of system. The audit list above isn't a hypothetical exercise; those were real bugs in my own running code that I found and fixed.
